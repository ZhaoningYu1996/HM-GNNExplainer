...
for epoch in range(1, 201):
    ...
    # print(out[mask].size())
    # print(data.y[mask].unsqueeze(dim=1).size())
...
# node_index = 300
# print(data.smiles_list[node_index])
# print(data.selected_idx[node_index-num_nodes])
# explanation = explainer(data.x, data.edge_index, index=node_index)
# print(f'Generated explanations in {explanation.available_explanations}')

# print(explanation.edge_mask.size())
# selected_edges = []
# selected_values = []
# threshold = 0.8
# max_index = -1
...
#             max_value = selected_values[i]
#             selected_motif = data_smiles[edge_list[1]]
#     elif node_index == edge_list[1]:
#         if selected_values[i] > max_value:
#             max_value = selected_values[i]
#             selected_motif = data_smiles[edge_list[0]]
#     else:
#         continue
# print(selected_motif)
# new_data, sparsity = convert_data(ori_dataset[data.selected_idx[node_index-num_nodes]], selected_motif, data_name)
# print(data)
# print(new_data)
...
# model_test.eval()
# with torch.no_grad():
#     out1 = model_test(ori_dataset[data.selected_idx[node_index-num_nodes]].x, ori_dataset[data.selected_idx[node_index-num_nodes]].edge_index)
#     out2 = model_test(new_data.x.float(), new_data.edge_index)

#     for batch in dataloader:
#         batch.to(device)
#         out = model_test(batch.x, batch.edge_index)
#     print(out1)
#     print(out2)
...
for id in tqdm(range(len(data.selected_idx))):
    ...
    num_graphs += 1
    # print(data.y[node_index])
    # print(out1[node_index])
    # print(out2[node_index])
    # print(out1[node_index].softmax(dim=0)[data.y[node_index]])
    # print(out2[node_index].softmax(dim=0)[data.y[node_index]])
    # print(stop)
